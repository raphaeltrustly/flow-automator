...
class PaymentFlow():
    custom_data = {'vipTier': "1"}
    selected_bank = "DEMO BANK"
    ...
    def __simulate_send_in_crb(self, ptx_list):
        event_ids = []
        for ptx in ptx_list:
        ...
        for event_id in event_ids:
            ...
            self.ach_simulator_event.ack()

    def __simulate_nsf_in_crb(self, ptx_list):
        event_ids = []
        for ptx in ptx_list:
            self.ach_simulator_payments.load()
            event_ids.append(self.ach_simulator_payments.get_event_id_by_ptx(ptx))            
        ...
        for event_id in event_ids:
            print(event_id)
            self.ach_simulator_event.load(event_id)
            self.ach_simulator_event.nsf('R01')
